...
int main(int argc, char** argv)
{
    ...
    int*       h_a = (int*) calloc(LENGTH, sizeof(int));       // a vector
    int*       h_b = (int*) calloc(LENGTH, sizeof(int));       // b vector
    int*       h_c = (int*) calloc(LENGTH, sizeof(int));       // c vector (a+b) returned from the compute device
    ...
    d_b  = clCreateBuffer(context,  CL_MEM_READ_ONLY,  sizeof(int) * count, NULL, &err);
    checkError(err, "Creating buffer d_b");

    d_c  = clCreateBuffer(context,  CL_MEM_WRITE_ONLY, sizeof(int) * count, NULL, &err);
    ...
    err = clEnqueueWriteBuffer(commands, d_b, CL_TRUE, 0, sizeof(int) * count, h_b, 0, NULL, NULL);
    checkError(err, "Copying h_b to device at d_b");

    err = clEnqueueWriteBuffer(commands, d_c, CL_TRUE, 0, sizeof(int) * count, h_c, 0, NULL, NULL);
    checkError(err, "Copying h_b to device at h_b");
    ...
    err = clEnqueueReadBuffer( commands, d_c, CL_TRUE, 0, sizeof(int) * count, h_c, 0, NULL, NULL );  
    if (err != CL_SUCCESS)
    {
    ...
    //    tmp = h_a[i] + h_b[i];     // assign element i of a+b to tmp
     ...
     //       correct++;
     //   else {
            printf(" tmp %d h_a %d h_b %d h_c %d \n",tmp, h_a[i], h_b[i], h_c[i]);
     //   }
    }
